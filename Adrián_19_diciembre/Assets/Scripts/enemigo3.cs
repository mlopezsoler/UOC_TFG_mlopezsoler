...
public class enemigo3 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        GameObject enemigo3 = GameObject.FindGameObjectWithTag("enemigo3");

        // Recupero el target
        GameObject destino = GameObject.FindGameObjectWithTag("Target3");
        ...
        if (transform.position == target.position)
        {
            ...
            if (destino.transform.position.x <= -2.0)
            {
                enemigo3.GetComponent<SpriteRenderer>().flipX = true;
            }
            else
            {
                enemigo3.GetComponent<SpriteRenderer>().flipX = false;
            }
